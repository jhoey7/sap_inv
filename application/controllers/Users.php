...
class Users extends CI_Controller {
	...
	function user($type="") {
		...
		if(strtolower($_SERVER['REQUEST_METHOD'])=="post") {
			...
			$columns = array("username","nama","alamat","telepon","email","status","user_role", "action");
	    	$aColumns = array("a.username","a.nama","a.alamat","a.telepon","a.email","b.reff_description","c.uraian");
		...
		} else if(in_array($type, array("add","update"))) {
			$this->load->model('m_users','model');
			$this->breadcrumbs = array("title"=>"Users","icon"=>"users","title_child"=>"Add User","url"=>'users/add');
	...
	function add() {
		...
		$this->load->model('m_users','model');
		if(strtolower($_SERVER['REQUEST_METHOD'])=="post") {
			$arrdata = $this->model->execute('user',$this->input->post('action'));
